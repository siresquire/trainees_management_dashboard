-- ── Tighten at-risk thresholds ───────────────────────────────────────────────
--
-- Previous rule flagged below 50% on any dimension. Updated per management:
--   • Labs: minimum 80% of lab tasks assigned cumulatively through the
--     cohort's current week
--   • KCs:  minimum 80%, same cumulative window
--   • Attendance: minimum 70% of sessions held so far
--
-- (Computation was already cumulative — weeks 0..current_week — only the
-- thresholds change.)
...
LANGUAGE sql STABLE SECURITY DEFINER AS $$
  ...
  WHERE
       (COALESCE(e.labs_expected, 0) > 0 AND COALESCE(d.labs_done, 0) < 0.8 * e.labs_expected)
    OR (COALESCE(e.kcs_expected, 0)  > 0 AND COALESCE(d.kcs_done, 0)  < 0.8 * e.kcs_expected)
    OR (COALESCE(s.total, 0)         > 0 AND COALESCE(a.att_done, 0)  < 0.7 * s.total)
