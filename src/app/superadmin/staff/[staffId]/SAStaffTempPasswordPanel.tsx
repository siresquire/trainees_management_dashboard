...
import { setStaffTempPassword } from "@/actions/profile";

export default function SAStaffTempPasswordPanel({ targetId }: { targetId: string }) {
  ...
  function handleGenerate() {
    ...
    startTransition(async () => {
      const result = await setStaffTempPassword(targetId);
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-5">
      ...
      <p className="text-xs text-slate-500 mb-4">
        Generate a one-time password to share with the staff member. They can change it after signing in.
      </p>
      ...
      {tempPassword ? (
        <div className="space-y-3">
          ...
          <p className="text-xs text-amber-600">
            Share this password securely. It can be used immediately to sign in.
          </p>
          <button
