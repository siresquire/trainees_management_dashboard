...
import { approveAccessRequest, denyAccessRequest } from "@/actions/staff";

export default function AccessRequestActions({ requestId }: { requestId: string }) {
  ...
  if (done === "approved") {
    return <span className="text-xs font-medium text-green-600 bg-green-50 px-2 py-0.5 rounded-full">Invite sent</span>;
  }
  if (done === "denied") {
    return <span className="text-xs font-medium text-slate-500 bg-slate-100 px-2 py-0.5 rounded-full">Denied</span>;
  }
